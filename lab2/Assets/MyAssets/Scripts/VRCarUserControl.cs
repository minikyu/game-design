...
public class VRCarUserControl : MonoBehaviour {
    ...
    private float Factor(Vector3 _originalPoint, Vector3 _point, Vector3 _zVector) {
		...
		float x0 = _originalPoint.x;
		float y0 = _originalPoint.y;
		float z0 = _originalPoint.z;
		...
		float Alpha = (xz * (x0 - xL) + yz * (y0 - yL) + zz * (z0 - zL)) / (xz * xz + yz * yz + zz * zz);





        return Alpha;   // Don't forget to replace it with your true result.
    }
	...
	private void OnButtonTwoChanged(object sender, ControllerInteractionEventArgs e)
	{
	...
	private void Awake() {
        ...
        if (leftController != null) {
			...
			leftControllerEvents.ButtonTwoPressed += OnButtonTwoChanged;
        ...
        if (rightController != null) {
			...
			rightControllerEvents.ButtonTwoPressed += OnButtonTwoChanged;
